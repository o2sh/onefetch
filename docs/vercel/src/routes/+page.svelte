<script lang="ts">
	...
	onMount(async () => {
		const response = await fetch('https://api.github.com/repos/o2sh/onefetch/releases/latest');
...
<header>
	...
	<p>
		<small>
			...
			<a href="https://github.com/o2sh/onefetch/tree/main/docs/vercel">github</a>
			| built with ❤ by
...
<main>
	<p>
		This page displays an ASCII preview for all the programming languages supported by onefetch.
		Like the binary, the data is sourced from the <a
			href="https://raw.githubusercontent.com/o2sh/onefetch/main/languages.yaml"
			><code>Languages.yaml</code></a
		> file, and the layout aims to replicate how the logo would appear inside a terminal.
	</p>
	<p>
		Suggestions and PRs are welcome at <a href="https://github.com/o2sh/onefetch"
			>github.com/o2sh/onefetch</a
		>
	</p>
	<div class="title">
	...
	<div class:hide={!showMenu}>
		<div class="checkbox-group">
			{#each languageTypes as type}
				<label for={type}>
					<input id={type} type="checkbox" value={type} bind:group={$filter.checkboxes} />
					{type}
				</label>
			{/each}
		</div>
